...
def classify_flight_delays():
    ...
    client.agile_data_science.prediction_tasks.insert_one(prediction_features)
...
def consume_prediction_from_kafka(unique_id):
    consumer = KafkaConsumer(
        'flight-delay-ml-response',
        bootstrap_servers=['kafka:9092'],
        auto_offset_reset='latest',
        enable_auto_commit=True,
        group_id='flask-predict-group',
    ...
    while True:
        ...
        for tp, msgs in records.items():
            for msg in msgs:
                ...
                if not prediction:
                    continue
                print(f"Mensaje recibido de Kafka: {prediction}")
                print(f"UUID esperado: {unique_id} - UUID recibido: {prediction.get('UUID')}")
...
def classify_flight_delays_realtime_response(unique_id):
    ...
    prediction = consume_prediction_from_kafka(unique_id)
